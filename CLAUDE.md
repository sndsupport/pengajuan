...
- Alur di atas berlaku untuk `type` `kendaraan`/`perlengkapan`/`gedung_fasilitas`. Untuk `type: "personalia"` (lembur/cuti/izin) alurnya beda: `diajukan → (dual approval spv DAN management, urutan bebas) → selesai` langsung — tidak ada `disetujui`/`siap_dikirim`/`on_proses_ga` dan tidak ada kirim ke GA, karena approver kedua yang menyelesaikan approval langsung menutup status ke `selesai`. Reject tetap sama (→ `perlu_revisi` → resubmit → `diajukan`, approval yang sudah ada di-reset ke `null`). **TTD & PDF (2026-09-22):** berbeda dari desain awal, setiap approval personalia (partial maupun final) sekarang tetap wajib mengisi tanda tangan digital (`spvApproval.signatureUrl`/`managerApproval.signatureUrl`, sama seperti `approverSignatureUrl` di alur operasional) — begitu approver kedua melengkapi dual approval, sistem otomatis generate PDF berisi kedua TTD (`lib/pdf/generateAndAttachPersonaliaPdf.ts`) dan mengisi `pdfUrl` pada submission yang sama (tanpa mengubah status, karena sudah `selesai`). Beda dengan alur operasional, tidak ada modal drag-posisi TTD di sini karena layout PDF personalia tetap/tidak ada tabel item dinamis — kedua tanda tangan langsung di-embed di template. Lihat "Ekspansi One Gate" di bawah.
...
submissions/{submissionId}
  ...
  pdfUrl: string | null           // link Google Drive (https://drive.google.com/...) — field ini dipakai juga oleh personalia, lihat di bawah
  ...
  spvApproval: { approverId: string; approverName: string; note: string | null; decidedAt: Timestamp; signatureUrl: string } | null
  managerApproval: { approverId: string; approverName: string; note: string | null; decidedAt: Timestamp; signatureUrl: string } | null
  // pdfUrl (field yang sama dengan operasional di atas) diisi otomatis begitu spvApproval
  // DAN managerApproval lengkap, berisi kedua TTD (lihat lib/pdf/generateAndAttachPersonaliaPdf.ts).
  // Status tidak berubah karena sudah "selesai".
...
- `submissions/{id}`: create diizinkan kalau `role == 'admin'` dan `requesterId == auth.uid` dengan status awal `diajukan`; read diizinkan untuk **siapa saja yang sudah login** (`isSignedIn()`, sejak 2026-09-04 — sebelumnya dibatasi pemilik/`spv`/`management`/`superadmin`). Update status dijaga per-transisi secara eksplisit di rules (resubmit setelah revisi, approve, reject, generate PDF, konfirmasi kirim ke GA, tandai selesai) — masing-masing punya syarat pelaku, status lama yang diizinkan, dan `hasOnly()` field yang boleh berubah. **Tidak ada Cloud Function di jalur ini** — rules-lah yang jadi satu-satunya penjaga.
- Personalia (`type == 'personalia'`) punya klausa create/update tambahan yang khusus scoped ke `type` ini (rules operasional untuk `kendaraan`/`perlengkapan`/`gedung_fasilitas` tidak diubah): create juga diizinkan untuk `spv` kalau `subType in ['cuti', 'izin']` (bukan `lembur`); update partial approval mengizinkan `spv`/`management` mengisi field approval miliknya sendiri (`spvApproval`/`managerApproval`, wajib menyertakan `signatureUrl` non-kosong) selama punya sendiri masih `null`, tanpa mengubah `status`, dan tidak boleh dilakukan pada submission milik sendiri (`requesterId != auth.uid`, mencegah self-approval untuk spv yang self-submit cuti/izin); update final approval mengizinkan approver kedua mengubah `status` ke `selesai` sekaligus mengisi approval-nya (juga wajib `signatureUrl`), hanya kalau approval yang lain sudah terisi. Ada satu klausa tambahan lagi khusus setelah `selesai`: salah satu dari kedua approver (`spvApproval.approverId` atau `managerApproval.approverId`) boleh mengisi/menimpa `pdfUrl` (hasOnly hanya field itu) — dipakai otomatis begitu dual approval lengkap, dan sebagai jalur retry manual kalau generate PDF gagal.
...
| `lib/submissions/reviewPersonaliaSubmission.ts` | Halaman Antrian Persetujuan, submission `type: "personalia"` | Approve (wajib TTD, sama seperti operasional) → isi `spvApproval`/`managerApproval` milik sendiri (termasuk `signatureUrl`); kalau approval yang lain sudah ada, sekalian set status `selesai`, lalu langsung panggil `generateAndAttachPersonaliaPdf` (gagal generate PDF tidak membatalkan approval yang sudah ter-commit — retry manual tersedia di halaman detail). Reject → sama seperti alur operasional |
| `lib/pdf/generateAndAttachSubmissionPdf.ts` + `lib/pdf/generateSubmissionPdfClient.ts` (`renderSubmissionBaseCanvas`/`compositeSignatureAndBuildPdf`) + `components/pdf/SignaturePlacementModal.tsx` | Antrian Persetujuan (saat approve) & halaman detail pengajuan (retry), khusus `kendaraan`/`perlengkapan`/`gedung_fasilitas` | Render template tanpa TTD approver ke satu kanvas, tampilkan sebagai preview yang bisa di-drag lewat `SignaturePlacementModal` supaya approver bisa taruh TTD-nya di posisi manapun, lalu composite ke kanvas + slice per halaman + jsPDF, upload ke Google Drive, update `pdfUrl` + status `siap_dikirim`. Approve tidak lagi auto-generate PDF di background — modal ini jadi satu-satunya jalur, dipakai baik langsung setelah approve maupun lewat retry manual |
| `lib/pdf/generateAndAttachPersonaliaPdf.ts` + `lib/pdf/generatePersonaliaSubmissionPdf.ts` | Dipanggil otomatis dari `reviewPersonaliaSubmission.ts` setelah dual approval lengkap, dan dari tombol retry di halaman detail (`type: "personalia"`, status `selesai`, tanpa `pdfUrl`) | Beda dari alur operasional: layout PDF personalia tetap (tidak ada tabel item dinamis), jadi kedua TTD (spv & management, sudah tersimpan di `spvApproval.signatureUrl`/`managerApproval.signatureUrl`) langsung di-embed di template dan di-render sekali jalan (tanpa modal drag-posisi) → slice per halaman + jsPDF, upload ke Google Drive, update `pdfUrl` (status tetap `selesai`, tidak ada transisi status) |
...
- **`personalia`** (`lembur`/`cuti`/`izin`): alur yang beda secara material — 1 dokumen upload (bukan items+tanda tangan), dual approval dari `spv` DAN `management` (bukan salah satu, masing-masing wajib TTD sendiri), auto-selesai tanpa tahap kirim-ke-GA (lihat varian di "Alur Status"). Begitu dual approval lengkap, PDF berisi kedua TTD digenerate otomatis (lihat catatan "TTD & PDF" di "Alur Status" di atas) — tapi tidak ada tahap kirim-ke-GA, `pdfUrl`-nya cuma jadi arsip formal, bukan bagian dari alur kirim WA (yang masih pakai `attachmentUrl`/dokumen asli, lihat `buildPersonaliaWaTemplate` di bawah). Modul terisolasi: `submitPersonaliaSubmission.ts` + `reviewPersonaliaSubmission.ts` (+ `generateAndAttachPersonaliaPdf.ts`/`generatePersonaliaSubmissionPdf.ts` untuk PDF), tidak menyentuh modul operasional yang sudah ada. `spv` jadi bisa mengajukan `cuti`/`izin` (sebagai karyawan) sekaligus tetap approve kategori lain — lihat tabel User Roles. Template WA-nya beda juga: `buildPersonaliaWaTemplate` di `lib/wa-template.ts`, ditujukan ke HC bukan GA.
